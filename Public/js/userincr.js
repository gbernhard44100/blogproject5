        ...
        return this.each(function () {
            ...
            edit.on('change', function (e) {
                ...
                if (0) {
                    ...
                    if (v > 0)
                        newdelta(OPS.mul, 1 + v / 100, 'inc');
                    else
                        newdelta(OPS.mul, 1 / (1 + v / 100), 'dec');
                } else if ('-' === v.substr(0, 1) && edit.data('min') >= 0) {
                    newdelta(OPS.add, -parseFloat(v), 'dec');
                } else if ('+' == v.substr(0, 1)) {
                    v = parseFloat(v.substr(1));
                    if (v < 0)
                        newdelta(OPS.add, -v, 'dec');
                    else
                        newdelta(OPS.add, v, 'inc');
                } else if ('*' == v.substr(0, 1)) {
                    v = parseFloat(v.substr(1));
                    if (v > 1)
                        newdelta(OPS.mul, v, 'inc');
                    else
                        newdelta(OPS.mul, 1 / v, 'dec');
                } else if ('/' == v.substr(0, 1)) {
                    v = parseFloat(v.substr(1));
                    if (v > 1)
                        newdelta(OPS.mul, v, 'dec');
                    else
                        newdelta(OPS.mul, 1 / v, 'inc');
                } else {
                    btn[1].focus();
                }
                limit_val();
